...
class SigunguActivity : AppCompatActivity(){
    ...
    private fun getAdaption() {
        ...
        RetrofitObject_sigungu.getApiService().getInfo("6260000","json").enqueue(object :
            Callback<Sigungu> {
            override fun onResponse(call: Call<Sigungu>, response: Response<Sigungu>) {
                setResponseText(response.code(), response.body())
                Toast.makeText(applicationContext, "success", Toast.LENGTH_SHORT).show()
            }

            override fun onFailure(call: Call<Sigungu>, t: Throwable) {
    ...
    private fun setResponseText(resCode: Int, body: Sigungu?) {
        // 상태별 text 설정
